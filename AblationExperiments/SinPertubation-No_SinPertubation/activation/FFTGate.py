...
class FFTGate(nn.Module):
    ...
    def __init__(self, gamma1=1.5, phi=0.1, history_len=16,
                 enable_history=True, use_skip=True, decay_mode="linear", gate_mode="FFT", perturbation_mode="no_sin"):     # gate_mode: "disable" Or "FFT" or "no_FFT" | perturbation_mode="sin" or "no_sin"
        super().__init__()
    ...
    def forward(self, x, epoch=0):
        ...
        if self.gate_mode == "disable":                 
            gate = torch.ones_like(x)
        elif self.gate_mode == "no_FFT":
            gate = torch.sigmoid(gamma1 * x)
        elif self.gate_mode == "FFT":
            gate = torch.sigmoid(gamma1 * x - freq_factor * freq_magnitude)
        else:
        ...
        # ✅ Main activation logic: gated + optional sinusoidal perturbation
        if self.perturbation_mode == "sin":
            activation = x * gate + 0.05 * torch.sin(self.phi * x)
        elif self.perturbation_mode == "no_sin":
            activation = x * gate
        else:
            raise ValueError(f"Invalid perturbation_mode: {self.perturbation_mode}")
